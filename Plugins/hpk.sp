 ...
 * =============================================================================
 *
 * Этот плагин подредактирован специально для игрового проекта Атомик
 *
 */
...
CreateConVars( )
{
	...
	g_Cvar_MaxPing = CreateConVar("sm_maxping", "120", "Max Ping for Players");
	...
	g_Cvar_MinPacketIn = CreateConVar("sm_minpacketin", "15.0", "Min packets in for Players");
...
public OnMapStart( )
{
	...
	CreateTimer(10.0, CheckLatency, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}
...
CheckClientForKick(client)
{
	if(g_PingCheck[client] >= g_MaxPingCheck)
    {
        ...
        KickClient(client, "[АТОМИК] Твой пинг (%d) очень большой. Макс. пинг: %d", g_Ping[client], GetConVarInt(g_Cvar_MaxPing));
        PrintToChatAll("[АТОМИК] Игрок %s кикнут из-за большого пинга. (%d)", name, g_Ping[client]);
    }
	...
	if(g_ChokeCheck[client] >= g_MaxPingCheck)
    {
        decl String:name[MAX_NAME_LENGTH];
        GetClientName(client, name, sizeof(name));
        KickClient(client, "[АТОМИК] Your choke (%f) is too high. Max choke: %f", g_Choke[client], GetConVarInt(g_Cvar_MaxChoke));
        PrintToChatAll("[АТОМИК] Игрок %s кикнут из-за большого пинга. (%f)", name, g_Choke[client]);
    }
	...
	if(g_LossCheck[client] >= g_MaxPingCheck)
    {
        decl String:name[MAX_NAME_LENGTH];
        GetClientName(client, name, sizeof(name));
        KickClient(client, "[АТОМИК] Your loss (%f) is too high. Max loss: %f", g_Loss[client], GetConVarInt(g_Cvar_MaxLoss));
        PrintToChatAll("[АТОМИК] Игрок %s кикнут из-за большого пинга. (%f)", name, g_Loss[client]);
    }
	...
	if(g_PacketInCheck[client] >= g_MaxPingCheck)
    {
        decl String:name[MAX_NAME_LENGTH];
        GetClientName(client, name, sizeof(name));
        KickClient(client, "Твои исходящие пакеты (%f) это очень мало. Min. Packets in: %f", 
			g_PacketsIn[client], GetConVarFloat(g_Cvar_MinPacketIn));
        PrintToChatAll("[АТОМИК] Игрок %s кикнут из-за малого количества пакетов. (%f)", name, g_PacketsIn[client]);
    }
